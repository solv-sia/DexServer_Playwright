// Superfilter: Búsqueda por Canal IPTV
...
test.describe('Búsqueda por superfiltro - Canal IPTV', () => {
  ...
  test('@CP58BPP Canal IPTV NO EN LA LISTA', async ({ page }) => {
    ...
    const count = await page.locator('#dexNetworkList dex-network-group').count();
    expect(count).toBeGreaterThanOrEqual(0);
    await page.screenshot({ path: 'screenshots/cp58bpp.png' });
  });
});
