// Superfilter: Búsqueda por Default Playlist
...
test.describe('Búsqueda por superfiltro - Default Playlist', () => {
  ...
  test('@CP59APP Default Playlist EN LA LISTA', async ({ page }) => {
    test.setTimeout(300000);
    const { networkPage } = await setup(page);
    await networkPage.clickSuperFilterPopUp();
    await networkPage.typeInSuperFilterField('Default Playlist', 0);
    ...
    await page.screenshot({ path: 'screenshots/cp59app.png' });
  });

  test('@CP59BPP Default Playlist NO EN LA LISTA', async ({ page }) => {
    test.setTimeout(300000);
    const { networkPage } = await setup(page);
    await networkPage.clickSuperFilterPopUp();
    await networkPage.typeInSuperFilterField('Default Playlist', 0);
    ...
    const count = await page.locator('#dexNetworkList dex-network-group').count();
    expect(count).toBeGreaterThanOrEqual(0);
    await page.screenshot({ path: 'screenshots/cp59bpp.png' });
  });
});
